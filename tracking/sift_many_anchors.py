#!/usr/bin/env python3
"""
SIFT anchors (N small patches) + local NCC
- Picks N best SIFT keypoints (with distance-based suppression), crops small patches around each,
  tracks them by NCC, then estimates a similarity transform from initial -> current anchor centers
  to propagate the GT-initialised ROI.
- Glare handling: --glare_mode off|mask|inpaint (+HSV thresholds); optional --clahe
- Metrics parity with KLT/TM: OP/DP, Success/Precision (+AUCs), DP@0.10·min(H,W) (native units)
- Poster-quality plots + CSV + markdown report, identical filenames/format to your other trackers.
"""
...
def interpolate(p1, p2, n):
    ...
    for i in range(1, n+1):
        a=i/(n+1.0)
        x=int((1-a)*p1[0]+a*p2[0]); y=int((1-a)*p1[1]+a*p2[1])
        pts.append((x,y))
    return pts
...
def nms_keypoints_by_radius(kps, radius):
    """Greedy keep-best by response with L2 distance >= radius."""
    if not kps: return []
    kps_sorted = sorted(kps, key=lambda k:k.response, reverse=True)
    kept = []
    r2 = radius * radius
    for k in kps_sorted:
        x,y = k.pt
        if all((x - kk.pt[0])**2 + (y - kk.pt[1])**2 >= r2 for kk in kept):
            kept.append(k)
    return kept
...
# ---------- main ----------
def main():
    p=argparse.ArgumentParser(description="SIFT anchors (N small) + NCC")
    p.add_argument("--video", required=True)
    p.add_argument("--gt_json", required=True)
    p.add_argument("--work_w", type=int, default=99999)
    p.add_argument("--n_anchors", type=int, default=10, help="number of SIFT anchors to track")
    p.add_argument("--box_size", type=int, default=31, help="patch size per anchor (pixels in work space)")
    p.add_argument("--pad", type=int, default=15, help="search pad around each anchor box (work space)")
    p.add_argument("--thresh", type=float, default=0.70, help="NCC accept threshold")
    ...
    p.add_argument("--kp_radius", type=float, default=20.0,
                   help="min distance between selected keypoints (work space pixels)")
    args=p.parse_args()
    ...
    # -------- SIFT detection (global), mask glare, NMS by radius, take top-N --------
    sift = cv2.SIFT_create()
    kp_mask = None if args.glare_mode=="off" else cv2.bitwise_not(gmask0)
    kps, _ = sift.detectAndCompute(gray0i, kp_mask)
    kps = nms_keypoints_by_radius(kps, radius=args.kp_radius)
    if len(kps) == 0:
        raise RuntimeError("No SIFT keypoints found; try --clahe or different glare_mode.")

    # pick up to N anchors, create small boxes centered at each keypoint
    kps_sorted = sorted(kps, key=lambda k:k.response, reverse=True)[:args.n_anchors]
    boxes=[]
    for k in kps_sorted:
        cx,cy = k.pt
        bx = int(round(cx - args.box_size/2)); by = int(round(cy - args.box_size/2))
        bx = max(0, min(bx, wW-args.box_size)); by = max(0, min(by, wH-args.box_size))
        boxes.append((bx,by,args.box_size,args.box_size))
    ...
    while True:
        ...
        for i,b in enumerate(boxes):
            v,(nx,ny)=search_patch(gi, tmpls[i], b, args.pad)
            if v < args.thresh:
                curr.append(None)
                continue
            matched += 1
            w,h = b[2], b[3]
            boxes[i] = (nx,ny,w,h)
            tmpls[i] = gi[ny:ny+h, nx:nx+w].copy()  # update template on accept
            ...
            cv2.rectangle(fr,(rx,ry),(rx+rw,ry+rh),(0,128,255),1)

        # need at least 3 matched anchors for a stable similarity fit
        pred_box=None
        ok_pts = [c for c in curr if c is not None]
        if len(ok_pts) >= 3:
            src = np.array(init_cent[:len(curr)], dtype=np.float32)
            dst = np.array([c if c is not None else [np.nan,np.nan] for c in curr], dtype=np.float32)
            # filter to matched indices only
            mask_idx = ~np.isnan(dst[:,0])
            src_m = src[mask_idx]; dst_m = dst[mask_idx]
            if len(src_m) >= 3:
                M,_ = cv2.estimateAffinePartial2D(src_m, dst_m, method=cv2.RANSAC, ransacReprojThreshold=3.0)
                if M is not None:
                    p = M.dot(np.array([t0_w[0], t0_w[1], 1.0]))
                    tx_w, ty_w = int(p[0]-tw_w/2), int(p[1]-th_w/2)
                    tx, ty = int(round(tx_w/scale)), int(round(ty_w/scale))
                    pred_box = (tx,ty,tw,th)
        else:
            lost = 1
            # re-seed: detect SIFT again (mask glare), re-initialise anchors
            kp_mask2 = None if args.glare_mode=="off" else cv2.bitwise_not(gmask)
            kps2,_ = sift.detectAndCompute(gi, kp_mask2)
            if kps2:
                kps2 = nms_keypoints_by_radius(kps2, radius=args.kp_radius)
                kps2 = sorted(kps2, key=lambda k:k.response, reverse=True)[:args.n_anchors]
                boxes=[]
                for k in kps2:
                    cx,cy = k.pt
                    bx = int(round(cx - args.box_size/2)); by = int(round(cy - args.box_size/2))
        ...
        if pred_box:
            ...
            tcx = px + pw/2.0; tcy = py + ph/2.0
            traj = interpolate((int(n0[0]), int(n0[1])), (int(tcx), int(tcy)), args.pts)
            for qx, qy in traj:
                cv2.circle(fr, (qx, qy), 2, (0,255,0), -1)
            cv2.circle(fr, (int(n0[0]), int(n0[1])), 4, (0,0,255), -1)
        ...
        cv2.putText(fr,f"SIFT+NCC (N={args.n_anchors})  FPS:{inst_fps:5.1f}  anchors:{matched}",
                    (10,60), cv2.FONT_HERSHEY_SIMPLEX,0.7,(255,255,255),2)
        if gt_box and (iou_v is not None):
            ok_op = iou_v > args.op_thr
            ok_dp = (cle_v is not None) and (cle_v < args.dp_thr)
            cv2.putText(fr, f"IoU:{iou_v:.2f}", (10,90), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                        (0,255,0) if ok_op else (0,0,255), 2)
            if cle_v is not None:
                cv2.putText(fr,f"CLE:{cle_v:.1f}px", (10,120), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                            (0,255,0) if ok_dp else (0,0,255), 2)

        cv2.imshow("Track", fr)
        if cv2.waitKey(1)==27: break

        # CSV row; "inliers" column = matched anchors count
        rows.append([
            frame_no, matched,
    ...
    dp10 = 0.10 * min(H,W)  # native units (explicit)
    ...
    if len(cle_series):
        ...
        plt.axhline(args.dp_thr, ls="--", label=f"DP thr {args.dp_thr}px (native)")
    ...
    if precision_curve.size:
        ...
        plt.xlabel("Distance threshold (px, native)"); plt.ylabel("Precision"); plt.ylim([0,1.02])
        plt.legend(frameon=False); plt.tight_layout()
        ...
        poster_axes(ax, "Distance threshold (px, native)", "Precision")
    ...
    with open(args.report_md, "w") as f:
        ...
        if ious.size:
            ...
            f.write(f"- Final DP (dist<{args.dp_thr:.0f}px native): {final_dp*100:.1f}%\n")
